...
void main(List<String> args) async {
  print('================================================================');
  print('  AirCanvas 100-Iteration Deep Stress & Precision Test Suite   ');
  print('================================================================\n');
  ...
  for (int run = 1; run <= 100; run++) {
    stdout.write('Run [${run.toString().padLeft(3, ' ')}/100]: Connecting... ');
    ...
    try {
      ...
      // Flood 100 high-frequency drawing packets per run
      int runPackets = 0;
      for (int i = 0; i < 100; i++) {
        final t = i / 100.0;
        final x = 0.1 + 0.8 * t;
        final y = 0.5 + 0.4 * sin(t * pi * 6);
        final event = InputEvent(
          type: i == 0
              ? InputEventType.pointerDown
              : (i == 99 ? InputEventType.pointerUp : InputEventType.pointerMove),
          x: x,
          y: y,
          pressure: 0.2 + 0.7 * cos(t * pi),
      ...
      totalSuccesses++;
      print('✅ OK (100/100 packets drawn)');
      await socket.close();
    } catch (e) {
      print('❌ Error: $e');
      totalFailures++;
  ...
  stopwatch.stop();
  print('\n================================================================');
  print('  100-Iteration Stress Test Final Summary Report');
  print('================================================================');
  print('Total Test Runs      : 100');
  print('Successful Runs      : $totalSuccesses / 100 (${(totalSuccesses).toStringAsFixed(1)}%)');
  ...
  if (totalSuccesses == 100) {
    print('🎯 100/100 PERFECT SUCCESS - AIR CANVAS IS 100% STABLE AND BUG-FREE!');
    exit(0);
  } else {
